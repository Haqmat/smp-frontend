...
const CHART_COLORS = ['#5A3E2B', '#a9927d', '#b8860b', '#5a3d09'];
        ...
        <div className="flex items-center gap-2 bg-accent p-1.5 rounded-xl text-sm font-medium">
          {(['today', 'this_week', 'this_month', 'this_year'] as const).map((p) => (
            <button
              key={p}
              onClick={() => setPeriod(p)}
              className={`px-3 py-1.5 rounded-lg transition-all capitalize ${period === p
      ...
      {loading && !data ? (
        ...
        <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="p-6 rounded-2xl lg:col-span-2 bg-card text-card-foreground border border-border">
              ...
              <div className="h-64">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={getMillingChartData()} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                    ...
                    <Legend />
                    <Bar dataKey="Grain" name="Input Grain Milled" fill="#a9927d" radius={[8, 8, 0, 0]} barSize={60} />
                    <Bar dataKey="Flour" name="Finished Flour Produced" fill="#5A3E2B" radius={[8, 8, 0, 0]} barSize={60} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </div>
            ...
            <div className="bg-card p-6 rounded-2xl text-card-foreground border border-border flex flex-col">
              <h3 className="text-lg font-bold text-foreground mb-4">
                    ...
                    {!data?.low_stock_alerts || data.low_stock_alerts.length === 0 ? (
                      ...
                      data.low_stock_alerts.map((alert, index) => (
                        <TableRow key={index} className="border-b border-gray-50 border-border/30">
                          ...
                          <TableCell className="text-right text-red-700 dark:text-red-400 font-semibold">{formatNumber(alert.current_stock)} kg</TableCell>
                          <TableCell className="text-right text-gray-500">{formatNumber(alert.threshold)} kg</TableCell>
                          <TableCell className="text-center">
                            <span className="inline-flex items-center rounded-md bg-red-50 dark:bg-red-950/30 px-2 py-1 text-xs font-medium text-red-700 dark:text-red-400 ring-1 ring-inset ring-red-700/10">
                              Low Stock
                            </span>
                          </TableCell>
                        </TableRow>
              ...
              <div>
                ...
                <div className="space-y-3">
                  <Button
                    ...
                    className="w-full justify-start rounded-xl bg-[#5A3E2B] hover:bg-[#5a3d09] text-white py-3 text-base"
                  >
                  ...
                  >
                    <ListPlus className="mr-2 h-5 w-5 text-[#5A3E2B]" />
                    Record Grain Intake
                  </Button>
                  <Button
                    variant="outline"
                    ...
                    className="w-full justify-start rounded-xl py-3 text-base border-border"
                  >
                    <BookOpen className="mr-2 h-5 w-5 text-[#5A3E2B]" />
                    View Reports
                  </Button>
                </div>
              </div>
